...
/*++
Routine Description:
    This routine is called when a driver first loads (si installa).  
	Its purpose is to initialize global state and then register with FltMgr to start filtering.
	E decidere una certa altitude
...
--*/
{
	...
	/*  The filter manager sets this flag to indicate that the minifilter driver 
		instance is being detached and that this postoperation callback routine is 
		being called to clean up the minifilter driver's completion context. 
		The postoperation callback should return FLT_POSTOP_FINISHED_PROCESSING. */
