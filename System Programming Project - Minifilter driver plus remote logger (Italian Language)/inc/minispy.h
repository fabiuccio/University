...
typedef struct _RECORD_DATA {
	...
	//USHORT			2 byte
	//UCHAR				1 byte
	//ULONG_PTR/FILE_ID 8 byte IA64 / 4 byte IA32 
	//PVOID				8 byte IA64 / 4 byte IA32
	...
	ULONG Flags; /* Nonostante non necessari teniamo memoria dei flags in 
					quanto è comunque necessario avere un allineamento a
					16 byte che richiederebbe l'adozione di un padding di
					6 bytes. In questo modo sfruttiamo almeno 4 dei 6 bytes
					contenendo informazioni sensate */
    UCHAR Reserved[2];	// Per allineamento in IA64 (16 bytes alignment)
